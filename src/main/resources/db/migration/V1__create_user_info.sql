...
ALTER TABLE
    `subscriptions_type`
ADD
    UNIQUE `subscriptions_type_product_key_unique`(`product_key`);

ALTER TABLE
    `user_payment_info`
ADD
    UNIQUE `user_payment_info_card_number_unique`(`card_number`);
...
INSERT INTO
    `subscriptions_type` (`name`, `access_months`, `price`, `product_key`)
VALUES
...
VALUES
    (
        'PROFESSOR',
        'Professores da plataforma - cadastro administrativo'
    );
...
VALUES
    (
        'ADMINISTRADOR',
        'Administrado da plataforma - cadastro administrativo'
    );
